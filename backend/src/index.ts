...
const allowedOrigins = [
  'http://localhost:5173',
  'http://localhost:3000',
  'https://uwe-ep.netlify.app/' // Replace with your exact Netlify URL (no trailing slash)
];

app.use(cors({
  origin: (origin, callback) => {
    // Allow requests with no origin (like Postman, mobile apps, or same-origin calls)
    if (!origin || allowedOrigins.indexOf(origin) !== -1) {
      callback(null, true);
    } else {
      callback(null, true); // Allows any origin while echoing back the exact requesting domain
    }
  },
  credentials: true,
